 ...
 * 需要注意的是{@link universecore.util.proxy.InvokeChains}是一条调用链，你可以多次对同一个方法执行addMethodProxy操作，而每一次添加，传入的superHandle都是上一次添加的调用链，并不是仅指向super方法的指针
 * @see universecore.util.proxy.InvokeChains
 * @author EBwilson
 * @since 1.2*/
@FunctionalInterface
public interface ProxyHandler<R, S>{
  ...
  R invoke(S self, InvokeChains<R, S> superHandle, Object[] param);
}
