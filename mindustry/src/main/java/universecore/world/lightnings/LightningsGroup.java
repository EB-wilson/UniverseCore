 ...
 * 注意这个容器应当被保存并持续使用，不应该随时回收，尽管实现了{@link Pool.Poolable}接口，但这个对象通常情况应当被重复使用而不是不断的释放又创建
 *
 * @since 1.5
 * @author EBwilson
 * @deprecated  可能不使用
...
public class LightningsGroup extends LightningContainer implements Drawc, Pool.Poolable{//TODO 未完成
  ...
  /**创建一个闪电组的实体，将设置一个默认的闪电生成器
   *
   * @param x 实体的初始x坐标
   * @param y 实体的初始y坐标
   * @param generator 默认闪电生成器
   * */
