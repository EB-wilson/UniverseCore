...
public class ObjectDataMonitor{
        ...
        modify.clicked(() -> {
          ...
          table.button(Core.bundle.get("misc.confirm"), () -> {
            ...
            if(input.getText().startsWith("@")){
              FieldHandler.setValueDefault(target, f.getName(), obj);
              buildObjectTable(target, f, object, table);
            }
            else if(input.getText().equals("null")){
              FieldHandler.setValueDefault(target, f.getName(), null);
              buildObjectTable(target, f, object, table);
            }
                ...
                public Dialog show(){
                  cont.table(table -> {
                    ...
                    table.table(Tex.button, t -> {
                      t.add(Core.bundle.format("debugModule.dataMonitor.varsDeleteConfirm", s + "(" + object + ")"));
                    }).height(250);
                    table.row();
                    table.table(t -> {
                      t.defaults().grow();
        ...
        TextButton b = t.button(Core.bundle.get("misc.methodInvoke"), () -> {
          ...
          try{
            ...
            if(!(params.length == 1 && params[0].equals("")) && params.length != paramType.length){
              Vars.ui.showErrorMessage(new IllegalArgumentException("Incorrect number of parameters(input: " + params.length + ", require: " + paramType.length).toString());
              return;
            }
  ...
  public class SetFlagDialog extends BaseDialog{
    ...
    public void show(Label text, String preString, VarStructure structure){
      if(!initialized){
        Vars.ui.showErrorMessage(Core.bundle.get("warning.noVarsSetError"));
        return;
      }
  ...
  private void setValue(Field field, Object target, String value){
    ...
    if(value.equals("null")){
      FieldHandler.setValueDefault(target, field.getName(), null);
      return;
    }
    
    if(value.startsWith("@")){
      Object result = vars.get(value.replaceFirst("@", ""));
      FieldHandler.setValueDefault(target, field.getName() , result);
    }
    else{
      try{
        ...
        FieldHandler.setValueDefault(target, field.getName(), maker.invoke(null, value));
      }catch(NoSuchMethodException | IllegalAccessException | InvocationTargetException e){
        Log.err(e);
      }
    }
  ...
  public static Seq<Field> getData(Class<?> clazz){
