      ...
      for(Element element : roundEnv.getElementsAnnotatedWith(annotation)){
        if(element instanceof TypeElement){
          ...
          for(AnnotationMirror mirror : element.getAnnotationMirrors()){
            if(mirror.getAnnotationType().toString().equals(Annotations.Entrust.class.getCanonicalName())){
              for(Map.Entry<? extends ExecutableElement, ? extends AnnotationValue> entry : mirror.getElementValues().entrySet()){
                switch(entry.getKey().getSimpleName().toString()){
                  case "blackList":
                    blackList = (int) ((Attribute.Constant) entry.getValue()).value != 0;
                    break;
                  case "extend":
                    extend = ((Attribute.Class) entry.getValue()).classType;
                    break;
                  case "implement":
                    for(Attribute attribute : ((Attribute.Array) entry.getValue()).getValue()){
                      interfaces.add(((Attribute.Class) attribute).classType);
                    }
                    break;
                }
              }
          ...
          if(entrustObj == null){
            entrustObj = maker.VarDef(
                maker.Modifiers(Modifier.PRIVATE | Modifier.FINAL),
            ...
            entrustObj.mods = maker.Modifiers(entrustObj.mods.flags | Modifier.FINAL);
