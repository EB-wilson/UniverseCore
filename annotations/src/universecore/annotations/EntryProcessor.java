...
public class EntryProcessor extends BaseProcessor{
  ...
  private void makeMethodEntry(Symbol.MethodSymbol symbol){
    ...
    String[] contextList = entry.context();
    Annotations.InsertPosition insert = Annotations.InsertPosition.END;

    for(Attribute.Compound mirror: symbol.getAnnotationMirrors()){
      if(!mirror.getAnnotationType().toString().equals(Annotations.MethodEntry.class.getCanonicalName())) continue;
      Attribute attr = mirror.member(names.fromString("insert"));
      if(attr == null) break;
      insert = Annotations.InsertPosition.valueOf(((Attribute.Enum)attr).value.getQualifiedName().toString());
      break;
    }
  ...
  private class GenMethod{
    ...
    private JCTree.JCMethodDecl genGetter(FieldEntry field){
      JCTree.JCMethodDecl method = maker.at(tree).MethodDef(
          maker.Modifiers(Modifier.PUBLIC),
          sym.name,
