...
public class FieldHandler{
  public static final ObjectMap<Class<?>, ObjectMap<String, Field>> fieldMap = new ObjectMap<>();
  ...
  public static Field getDeclaredFieldSuper(Class<?> clazz, String name) throws NoSuchFieldException{
    Field result = fieldMap.get(clazz, ObjectMap::new).get(name);
    ...
    while(current != Object.class){
      ...
      result = checking.find(f -> f.getName().equals(name));
      if(result != null){
        fieldMap.get(clazz).put(name, result);
        return result;
      }
