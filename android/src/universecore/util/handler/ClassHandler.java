...
import universecore.util.classes.AndroidGeneratedClassLoader;
import universecore.util.classes.BaseDynamicClassLoader;
import universecore.util.classes.DexLoaderFactory;
...
public class ClassHandler extends BaseClassHandler{
  static BaseDynamicClassLoader dynamicLoader = DexLoaderFactory.generateClassLoader(Vars.mods.mainLoader());
  ...
  public <T> BaseProxy<T> getProxy(Class<T> clazz, String name){
    return (BaseProxy<T>) proxies.get(clazz, ObjectMap::new).get(name, () -> new AndroidProxy<>(clazz, getCurrentLoader(), this));
  }
